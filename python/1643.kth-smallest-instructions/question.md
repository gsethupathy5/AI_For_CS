# [1643. Kth Smallest Instructions][link] (Hard)
...
travel **right** and **down**. You are going to help Bob by providing **instructions** for him to
reach `destination`.
...
However, Bob is very picky. Bob has a lucky number `k`, and he wants the `kᵗʰ` **lexicographically
smallest instructions** that will lead him to `destination`. `k` is **1-indexed**.

Given an integer array `destination` and an integer `k`, return the  `kᵗʰ`**lexicographically
