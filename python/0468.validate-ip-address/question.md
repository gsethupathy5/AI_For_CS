# [468. Validate IP Address][link] (Medium)
...
**A valid IPv4** address is an IP in the form `"x₁.x₂.x₃.x₄"` where `0 <= xᵢ <= 255` and `xᵢ`
**cannot contain** leading zeros. For example, `"192.168.1.1"` and `"192.168.1.0"` are valid IPv4
...
- `1 <= xᵢ.length <= 4`
- `xᵢ` is a **hexadecimal string** which may contain digits, lowercase English letter ( `'a'` to
`'f'`) and upper-case English letters ( `'A'` to `'F'`).
